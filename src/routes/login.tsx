...
import { Lock, User, Eye, EyeOff, ShoppingBasket, Wallet, AlertCircle, Loader2, KeyRound } from "lucide-react";
...
export const Route = createFileRoute("/login")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Acesse sua lista de compras e controle financeiro privado." },
      { property: "og:title", content: "Entrar — Minha Lista & Finanças" },
      { property: "og:description", content: "Área privada da sua lista de compras e gestão financeira." },
    ...
    <div className="relative min-h-dvh w-full flex items-center justify-center bg-background p-4 sm:p-6 overflow-hidden">
      ...
      <div className="relative w-full max-w-sm rounded-3xl border border-border/80 bg-surface/90 backdrop-blur-xl p-6 sm:p-8 shadow-2xl space-y-6 animate-rise">
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          {/* Alerta de Erro */}
          {error && (
            <div className="flex items-center gap-2 p-3 rounded-xl border border-destructive/30 bg-destructive/10 text-destructive text-xs font-semibold animate-in fade-in slide-in-from-top-1">
              <AlertCircle className="size-4 shrink-0" />
              <span>Login ou senha incorretos. Tente novamente.</span>
            </div>
          )}
